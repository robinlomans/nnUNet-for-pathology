...
class nnUNetTrainer_WSD_undefined_dataloader(nnUNetTrainer):
    def __init__(
        ...
        self.iterator_template = (
            f"wsd_{self.label_sampling_strategy}_iterator_{self.aug}_aug"
        )
        ###
    ...
    def do_split(self):
        if isfile(
            join(nnUNet_preprocessed, self.plans_manager.dataset_name, "splits.json")
        ):
            splits_json_path = join(
                nnUNet_preprocessed, self.plans_manager.dataset_name, "splits.json"
            )
            ...
            files_json = load_json(
                join(nnUNet_preprocessed, self.plans_manager.dataset_name, "files.json")
            )
            ...
            for fold in range(5):
                ...
                fold_dict = {"training": train_split, "validation": val_split}
    ...
    def get_dataloaders(self, subset=False, sample_double=False, cpus=10):
        ...
        iterator_template_path = join(
            os.path.dirname(__file__), f"{self.iterator_template}_template.json"
        )
        ...
        if self.time or subset:
            ...
            fold_split_dict = {
                "training": fold_split_dict["training"][-10:],
                "validation": fold_split_dict["validation"][-5:],
            }
        copy_path = "/home/user"
        ...
        spacing = (
            self.dataset_json["spacing"] if "spacing" in self.dataset_json else 0.5
        )
        load_images = (
            self.dataset_json["load_images"]
            if "load_images" in self.dataset_json
            else True
        )
        ...
        if self.sample_double or sample_double:
            ...
            extra_ds_sizes = ds_sizes[1:]
            ds_shapes = tuple(
                [tuple([batch_size] + ds_shape) for ds_shape in ds_shapes]
            )
            extra_ds_shapes = ds_shapes[1:]
        ...
        seed = int(
            str(time())[-3:]
        )  # Taking random seed to prevent the idea that you are using a specific seed, which doesnt work anymore if your training gets interrupted and you continue training afterwards
        ...
        if self.aug == "nnunet":
            nnunet_callback_idx = [
                fill_template["batch_callbacks"][i]["*object"].split(".")[-1]
                for i in range(len(fill_template["batch_callbacks"]))
            ].index("nnUnetBatchCallback")
            fill_template["batch_callbacks"][nnunet_callback_idx][
                "patch_size_spatial"
            ] = patch_size
        ...
        self.val_config["wholeslidedata"]["default"]["batch_callbacks"] = (
            self.val_config["wholeslidedata"]["default"]["batch_callbacks"][-1:]
        )  # remove data augmentation for validation
        ...
        class WholeSlidePlainnnUnetBatchIterator(BatchIterator):
            def __next__(self):
                ...
                data = torch.FloatTensor(x_batch.transpose(0, 3, 1, 2) / 255.0).to(
                    device
                )
        ...
        class WholeSlidePlainnnUnetBatchIteratorTIME(BatchIterator):
            def __next__(self):
                ...
                print(
                    "Time taken TOTAL:\t\t\t\t\t\t\t\t\t\t",
                    line1_time + line2_time + line3_time,
                )
        ...
        if self.time:
            iterator_class = (
                WholeSlidePlainnnUnetHalfCropBatchIteratorTIME
                if self.sample_double
                else WholeSlidePlainnnUnetBatchIteratorTIME
            )
        else:
            iterator_class = (
                WholeSlidePlainnnUnetHalfCropBatchIterator
                if self.sample_double
                else WholeSlidePlainnnUnetBatchIterator
            )
        ...
        tiger_train_batch_iterator = create_batch_iterator(
            mode="training",
        ...
        tiger_val_batch_iterator = create_batch_iterator(
            mode="validation",
    ...
    def _build_loss(self):
        ...
        if self.label_manager.has_regions:
            loss = DC_and_BCE_loss(
                ...
                use_ignore_label=0
                if self.ignore0
                else self.label_manager.ignore_label is not None,
            ...
            loss = DC_and_CE_loss(
                ...
                weight_ce=1,
                weight_dice=1,
                ignore_label=0 if self.ignore0 else self.label_manager.ignore_label,
                dice_class=MemoryEfficientSoftDiceLoss,
            )
    ...
    def on_train_start(self):
        ...
        save_json(
            self.splits_json,
            join(self.output_folder_base, "splits.json"),
            sort_keys=False,
        )
        save_json(
            self.train_config,
            join(self.output_folder_base, "train_config.json"),
            sort_keys=False,
        )
        save_json(
            self.val_config,
            join(self.output_folder_base, "val_config.json"),
            sort_keys=False,
        )
        ...
        try:
            ...
            for epoch in range(self.current_epoch, self.num_epochs):
                ...
                for batch_id in range(
                    15 if self.time else self.num_iterations_per_epoch
                ):
                ...
                with torch.no_grad():
                    ...
                    val_outputs = []
                    for batch_id in range(
                        15 if self.time else self.num_val_iterations_per_epoch
                    ):
                        val_outputs.append(
