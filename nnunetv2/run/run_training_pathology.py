    ...
    if num_gpus > 1:
        ...
        if (
            len(
                [
                    item
                    for item in os.listdir(nnunet_trainer.output_folder)
                    if item.endswith("checkpoint_latest.pth")
                ]
            )
            > 0
        ):
        ...
        if nnunet_trainer.wandb:
            ...
            if continue_training:
                run = wandb.init(
                    project=f"{nnunet_trainer.plans_manager.dataset_name}",
...
if __name__ == "__main__":
    ...
    print(
        f"[RUNNING TRAINING] dataset {args.dataset_name_or_id}, 2d model, FOLD {args.fold}, PRETRAINED WEIGHTS {args.pretrained_weights}, and pathology trainer and planner"
    )
    run_training(
